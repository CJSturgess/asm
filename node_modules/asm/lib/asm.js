...
asm.createServer = function(ram, callback) {
    var options = {
        "options": {
          "server-name": "An AnchorHosting Minecraft Server"  
        },
        "ram": ram
    };
    asm.server.create(options, function(err) {
        if (err) {
            ...
            fs.writeFile(path.join(asm.server.dir, "minecraft", "server", "eula.txt"), 'eula=true', function(err) {
                if (err) console.log("[createServer] An error occurred while writing eula.txt.".red);
                callback(false);
            });
            callback(true);
        }
